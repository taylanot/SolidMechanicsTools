...
#        cells = np.vstack([cell.data for cell in mesh.cells if \
#                cell.type==cell_type])
#        data = np.hstack([mesh.cell_data_dict["gmsh:physical"][key] for key in\
#                mesh.cell_data_dict["gmsh:physical"].keys() if key==cell_type])
...
#    
#    dim = (np.sum(np.max(mesh.points, axis=0) - np.min(mesh.points, axis=0) > \
#            1e-15))
#
#    if dim == 2:
